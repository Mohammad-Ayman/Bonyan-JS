...
const notStartedBtn = document.getElementById("not-started-btn");
...
const addEntry = () => {
  ...
  newLi.innerHTML = `<input type="text" name="" id="" placeholder="Type a task..."/>
...
const deleteEntry = (newLi) => {
  const newDeleteBtn = newLi.querySelector(".delete-icon");
  newDeleteBtn.addEventListener("click", deleteHandler);
};

// Handler functions
const notStartedBtnHandler = () => {
  renderEntry(notStartedBtn);
};

const inProgressBtnHandler = () => {
  renderEntry(inProgressBtn);
};

const completedBtnHandler = () => {
  renderEntry(completedBtn);
};
...
// eventListeners
notStartedBtn.addEventListener("click", notStartedBtnHandler);
